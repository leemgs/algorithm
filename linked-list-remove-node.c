...
void removeNode(struct NODE *target, int data) //  동일한 데이타 값이 있는 노드를 찾아서 삭제하는 함수 
{
	struct NODE *prev = target;
	struct NODE *temp = target;
	while (temp->next != NULL) // 마지막 노드까지 순차 검색
	{
		prev = temp;  // 이전 노드의 정보를 기억하고 있어야 한다. 
		temp = temp->next;
		if (temp->data == data){ // 동일한 데이타 값을 가지는 노드인지 체크하기 
			prev->next = temp->next;  // 동일한 데이타 값을 가지는 노드의 연결(link)을 끊어버리자. 
			break; // 동일한 데이타 값을 가지는 노드를 찾았다면 while문은 빠져나가자.!!!
		}
	}
}
...
int main(void)
{
	...
	head->next = NULL; 	// 주의: 머리 노드는 데이터를 저장하지 않음.이유는 Pointer(*) 프로그래밍이기 때문임. 
